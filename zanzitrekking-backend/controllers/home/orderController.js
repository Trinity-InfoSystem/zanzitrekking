...
const { autoCompleteTrips, checkOrderTripsCompletion } = require('../../utilities/autoCompleteTrips')
...
class OrderController {
  ...
  createOrder = async (req, res) => {
    const debug = debugNamespace('order:createOrder')
    ...
    try {
      ...
      if (!customerId || !cartItems || !personalInfo || !paymentInfo) {
        return responseReturn(res, 400, {
          error: 'Missing required fields: customerId, cartItems, personalInfo, paymentInfo'
        })
      }
      ...
      for (const item of cartItems) {
        ...
        if (trip.pricingType === 'yearRound') {
          ...
          if (applicableSeason) {
            ...
            logger.warn('No matching season found for trip date, using regular prices')
            pricePerPerson = this.calculatePricePerPerson(
              trip.regularPrices,
              travelersNumber,
              selectedCategory,
      ...
      try {
        ...
        debug('Payment Option:', paymentOption)
        debug('Deposit Amount:', depositAmount)
        debug('Total Amount:', totalAmount)
        debug('Payment Info Object:', JSON.stringify(paymentInfo, null, 2))
        ...
        debug('FRONTEND_URL env:', process.env.FRONTEND_URL || 'not set')
        ...
        debug('✅ WeTravel payment link created successfully')
      } catch (error) {
        debug('❌ Error generating WeTravel payment link:', error.message)
      ...
      debug('Create order error:', error)
    ...
    childrenAges.forEach((age) => {
      if (age !== null && age !== undefined) {
        ...
        if (age >= 5 && age < 12) {
          ...
          childDiscountRate = 0.1 // 10% discount for ages 12-15
        }
  ...
  updateOrderPricing = async (req, res) => {
    ...
    try {
      ...
      if (trip.pricingType === 'yearRound') {
        // Use regular prices for year-round pricing with selected category
        pricePerPerson = this.calculatePricePerPerson(trip.regularPrices, travelersNumber, selectedCategory)
      } else if (trip.pricingType === 'seasonal') {
  ...
  updateOrderStatus = async (req, res) => {
    ...
    try {
      ...
      if (itemIndex !== undefined && itemStatus) {
        const validItemStatuses = ['pending', 'confirmed', 'in-progress', 'completed', 'cancelled']
  ...
  getAllOrders = async (req, res) => {
    try {
      ...
      const totalRevenue = allOrdersForSummary.reduce((sum, order) => sum + (order.totalAmount || 0), 0)
      const pendingOrders = allOrdersForSummary.filter((order) => order.orderStatus === 'pending').length
      const completedOrders = allOrdersForSummary.filter((order) => order.orderStatus === 'completed').length
  ...
  updatePaymentStatus = async (req, res) => {
    ...
    try {
      ...
      const normalizedStatus = paymentStatus === 'review' ? 'processing' : paymentStatus
      ...
      const customerEmail = order.personalInfo?.email || order.customerId?.email
      ...
      if (customerEmail) {
        ...
        logger.warn(`[Email] ⚠️ No email found for order ${order.orderNumber}, skipping email notification`)
      }
      await Promise.allSettled([
        delPattern(`customer:${order.customerId}:orders:history:*`),
        redis.del(`customer:${order.customerId}:orders:statistics`),
  ...
  getCustomerOrderStatistics = async (req, res) => {
    ...
    try {
      ...
      const pendingOrders = customerOrders.filter((order) => order.orderStatus === 'pending').length
      const confirmedOrders = customerOrders.filter((order) => order.orderStatus === 'confirmed').length
      const preparingOrders = customerOrders.filter((order) => order.orderStatus === 'preparing').length
      const inProgressOrders = customerOrders.filter((order) => order.orderStatus === 'in-progress').length
      const completedOrders = customerOrders.filter((order) => order.orderStatus === 'completed').length
      const cancelledOrders = customerOrders.filter((order) => order.orderStatus === 'cancelled').length
      const refundedOrders = customerOrders.filter((order) => order.orderStatus === 'refunded').length

      // Calculate budget statistics
      // Total spent should only include orders with completed status AND completed payment
      const totalSpent = customerOrders
        .filter((order) => order.orderStatus === 'completed' && order.payment?.status === 'completed')
        .reduce((sum, order) => sum + (order.totalAmount || 0), 0)

      const totalBudget = customerOrders.reduce((sum, order) => sum + (order.totalAmount || 0), 0)
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const monthBudget = monthOrders.reduce((sum, order) => sum + (order.totalAmount || 0), 0)
      ...
      const totalTrips = customerOrders.reduce((sum, order) => sum + (order.cartItems?.length || 0), 0)
      const completedTrips = customerOrders.reduce((sum, order) => {
        return sum + (order.cartItems?.filter((item) => item.itemStatus === 'completed').length || 0)
      }, 0)
