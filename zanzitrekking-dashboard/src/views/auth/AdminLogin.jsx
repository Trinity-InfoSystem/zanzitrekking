...
const AdminLogin = () => {
  ...
  const { loader, errorMessage, successMessage, userInfo, isInitialized } = useSelector(
    (state) => state.auth,
  );
  ...
  const submit = (e) => {
    e.preventDefault();
    // Clear any previous error/success messages before attempting login
  ...
  useEffect(() => {
    ...
    // Only navigate if we have both successMessage AND userInfo
    // This prevents navigation on failed logins
    if (successMessage && userInfo) {
      toast.success(successMessage);
      dispatch(clearMessage());
      navigate("/admin/dashboard");
    }
  }, [errorMessage, successMessage, userInfo, navigate, dispatch]);
            ...
            <form className="mt-8 space-y-6" onSubmit={submit}>
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="text-sm">
                  <Link
                    to="/forgot-password"
                    className="font-medium text-secondary transition duration-150 hover:text-secondary-600"
                  >
                    Forgot password?
                  </Link>
                </div>
