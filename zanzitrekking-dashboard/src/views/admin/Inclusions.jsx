...
const Inclusions = () => {
  ...
  const {
    ...
    inclusion,
    totalInclusions,
  } = useSelector((state) => state.inclusion);
  ...
  function validateForm() {
    ...
    if (!name) {
      errors.name = "Inclusion name is required.";
      valid = false;
    }
  ...
  function add_inclusion(e) {
    e.preventDefault();
    if (!validateForm()) return;

    if (inclusionId) {
      dispatch(inclusion_update({ name, inclusionId }));
    } else {
      dispatch(inclusionAdd({ name }));
    }
    setShow(false);
  }
  ...
  useEffect(() => {
    ...
    if (successMessage) {
      ...
      dispatch(get_inclusions({ parPage, currentPage, searchValue }));

      navigate("/admin/dashboard/inclusions");
      setName("");
      dispatch(clearMessage());
    }
  }, [
  ...
  useEffect(() => {
    ...
    dispatch(get_inclusions(obj));
  }, [searchValue, currentPage, parPage, sort, dispatch]);

  useEffect(() => {
    if (inclusionId) {
      dispatch(get_inclusion(inclusionId));
    }
  }, [inclusionId, dispatch]);

  useEffect(() => {
    if (inclusion && inclusion.name) {
      setName(inclusion.name);
    }
  }, [inclusion]);

  useEffect(() => {
    return () => {
      dispatch(clearInclusion());
    };
  }, [dispatch]);
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedIds(inclusions.map((d) => d._id));
    } else {
      setSelectedIds([]);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-neutral-50 via-white to-primary-50/30 p-4 md:p-5">
      ...
      <ConfirmModal
        open={confirmOpen}
        message="Are you sure you want to delete this inclusion?"
        onCancel={() => {
        ...
        onConfirm={() => {
          if (toDeleteId) {
            dispatch(delete_inclusion(toDeleteId));
            setConfirmOpen(false);
            setToDeleteId(null);
          }
        }}
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to delete ${selectedIds.length} inclusions?`}
        onCancel={() => setBulkConfirmOpen(false)}
        onConfirm={() => {
          if (selectedIds.length > 0) {
            dispatch(delete_inclusions(selectedIds)).then(() => {
      ...
      <div className="mx-auto max-w-7xl">
        <HeaderText title="Inclusions Management" />
        ...
        <div className="mb-6 flex items-center justify-end lg:hidden">
          ...
          >
            ...
            <span>{inclusionId ? "Edit" : "Add New"}</span>
          </button>
        </div>
                        ...
                        {!isViewer(role) && (
                          <td className="px-6 py-4">
                            <div className="flex gap-3">
                              <Link
                                to={`/admin/dashboard/edit-inclusion/${d._id}`}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      ...
                      <h2 className="text-xl font-bold text-white">
                        {inclusionId ? "Edit Inclusion" : "Add New Inclusion"}
                      </h2>
                    </div>
                    <button
                    ...
                    <div className="mb-6">
                      ...
                      <div className="relative">
                        <input
                          ...
                          placeholder="Enter inclusion name (e.g., Free WiFi, Breakfast)"
                        />
                        {errors.name && (
