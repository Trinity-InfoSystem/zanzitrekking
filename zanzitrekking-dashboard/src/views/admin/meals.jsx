...
import { FaEdit, FaTrash, FaPlus, FaCoffee } from "react-icons/fa";
...
import {
  ...
  mealAdd,
  delete_meals,
} from "../../store/Reducers/mealReducer";
...
const Meals = () => {
  ...
  const { loader, successMessage, errorMessage, meals, meal, totalMeals } =
  ...
  function add_meal(e) {
    ...
    if (mealId) {
      dispatch(meal_update({ name, mealId }));
    } else {
      dispatch(mealAdd({ name }));
    }
    setShow(false);
  }
  ...
  useEffect(() => {
    ...
    if (successMessage) {
      ...
      dispatch(get_meals({ parPage, currentPage, searchValue }));
      if (mealId) {
        dispatch(get_meal(mealId));
      }
      setName("");
      dispatch(clearMessage());
    }
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      setSelectedIds(meals.map((d) => d._id));
    } else {
      setSelectedIds([]);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-neutral-50 via-white to-primary-50/30 p-4 md:p-5">
      ...
      <ConfirmModal
        ...
        onConfirm={() => {
          if (toDeleteId) {
            dispatch(delete_meal(toDeleteId));
            setConfirmOpen(false);
            setToDeleteId(null);
          }
        }}
      ...
      <ConfirmModal
        ...
        message={`Are you sure you want to delete ${selectedIds.length} meals?`}
        onCancel={() => setBulkConfirmOpen(false)}
        onConfirm={() => {
          if (selectedIds.length > 0) {
            dispatch(delete_meals(selectedIds)).then(() => {
      ...
      <div className="mx-auto max-w-7xl">
        <HeaderText title="Meals Management" />
        {/* Mobile Header */}
                        ...
                        {!isViewer(role) && (
                          <td className="px-6 py-4">
                            <div className="flex gap-3">
                              <Link
                                to={`/admin/dashboard/edit-meal/${d._id}`}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      <div className="rounded-lg bg-white p-2">
                        <FaCoffee className="text-lg text-secondary" />
                      </div>
