    ...
    <div ref={containerRef} className="flex-1 overflow-y-auto p-4">
      {Object.keys(groupedMessages).length > 0 ? (
              ...
              <div className="space-y-3">
                {dateMessages.map((message, idx) => (
                  <MessageBubble
                    key={idx}
                    message={message}
                    isCurrentUser={message.senderModel === "Admin"}
                    userInfo={userInfo}
        ...
        <EmptyState
          ...
          description={`Start the conversation by sending a message to ${currentCustomer.name}.`}
        />
      )}

      {isTyping && <TypingIndicator name={currentCustomer.name} />}
    </div>
  );
};
